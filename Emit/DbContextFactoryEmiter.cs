    ...
    class DbContextFactoryEmiter
    {
        ...
        public static Func<DbDataReader, IList<T>> EmitResultListFactory<T>(DbDataReader reader, IModelManager modelManager)
        {
           ...
           var method = new DynamicMethod("_$ResultListFactory_" + typeof(T).Name + _DynamicMethodNumber.ToString(),
            ...
            cacheEmmiter.EmitCacheInit(gen, meta);

            gen.Emit(OpCodes.Newobj, cacheEmmiter.ResultCtor); // [ items ]
            ...
            EmitSimpleProp(gen, meta.id_prop);
            ...
            //if (!cacheEmmiter.HasCache(meta))
                gen.Emit(OpCodes.Call, cacheEmmiter.ResultAddMethod);   // items.Add(item) => [items]
            //else
        ...
        private static void EmitReadAndSetSimpleProp(ILGenerator gen, prop_meta metaProp)
        {
            ...
            var readerMethod = ReaderMethodFromType(Helper.GetNonNullableType(propType), readerArgTypes);

            gen.Emit(OpCodes.Ldc_I4, metaProp.ordinal);// [items, items, item, item, reader, i_ordinal]

            gen.Emit(OpCodes.Callvirt, readerMethod); // [items, items, item, item, propValue]

            EmitNullableCtor(gen, propType);
        ...
        internal static MethodInfo ReaderMethodFromType(Type propType, Type[] readerArgTypes)
        {
            MethodInfo method;
        ...
        class EntityCacheEmitter
        {
            ...
            public void EmitCacheInit(ILGenerator gen, type_meta meta)
            {
                var types = meta.get_types_with_cache();
            ...
            public void EmitNewObject(ILGenerator gen, type_meta meta)
            {
                ...
                if (meta.descriminator_prop != null)
                {
                    ...
                    gen.Emit(OpCodes.Callvirt, ReaderMethodFromType(typeof(int), readerArgTypes)); // [items, items, descriminatorValue]
            ...
            public void EmitNewOrGetFromCache(ILGenerator gen, type_meta meta, type_meta_base impl) // [...] => [..., item]
            {
                ...
                Locals locals = _locals_cache[meta.type];
                {
                    ...
                    gen.Emit(OpCodes.Callvirt, ReaderMethodFromType(idType, readerArgTypes)); // => [ ..., cache, id]
            ...
            static void EmitAddToCache(ILGenerator gen, Locals locals, prop_meta id_prop)
            {
                ...
                gen.Emit(OpCodes.Callvirt, ReaderMethodFromType(GetMemberType(id_prop.prop), readerArgTypes)); // => [ ..., cache, id]
        ...
        public static DynamicMethod EmitValueFactory(Type type, string prefix, bool emitRead) 
        {
            ...
            else
            {
                ...
                Emit.DbContextFactoryEmiter.EmitNullableCtor(gen, type);
            }
